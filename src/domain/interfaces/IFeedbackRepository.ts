...
export interface IFeedbackRepository extends IBaseRepository<Document> {
    ...
    getPaginatedFeedbacksByInterviewer(interviewerId: string, page: number, limit: number, searchTerm: string, sortBy: string): Promise<{ feedbacks: Feedback[], total: number }>;
}