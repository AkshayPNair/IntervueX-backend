...
export interface IBookingRepository extends IBaseRepository<Document> {
    ...
    getInterviewerBookingsPaginated(interviewerId: string, page: number, limit: number, status: BookingStatus, search: string): Promise<{ bookings: Booking[], total: number, userMap: Map<string, any> }>;
