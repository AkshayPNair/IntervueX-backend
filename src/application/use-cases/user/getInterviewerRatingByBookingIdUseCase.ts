...
export class GetInterviewerRatingByBookingIdUseCase implements IGetInterviewerRatingByBookingIdService {
    constructor(
        private _feedbackRepository: IFeedbackRepository
    ) { }

    async execute(userId: string, bookingId: string): Promise<InterviewerFeedbackResponseDTO> {
        try {
